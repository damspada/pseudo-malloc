...
void test_simple_allocations_metabuddy() {
    ...
    // Try allocating some memory
    void* ptr1 = BuddyAllocator_malloc_metabuddy(allocator, 100);
    check(ptr1 != NULL, "allocated 100 bytes");

    void* ptr2 = BuddyAllocator_malloc_metabuddy(allocator, 256);
    check(ptr2 != NULL, "allocated 256 bytes");

    void* ptr3 = BuddyAllocator_malloc_metabuddy(allocator, 50);
    check(ptr3 != NULL, "allocated 50 bytes");
    ...
    // Free everything
    BuddyAllocator_free_metabuddy(allocator, ptr1);
    BuddyAllocator_free_metabuddy(allocator, ptr2);
    BuddyAllocator_free_metabuddy(allocator, ptr3);
...
void test_allocation_patterns_metabuddy() {
    ...
    for (int i = 0; i < 10; i++) {
        small_blocks[i] = BuddyAllocator_malloc_metabuddy(allocator, 64);
        if (small_blocks[i] != NULL) {
            successful_allocs++;
        }
    }
    ...
    for (int i = 4; i >= 0; i--) {
        if (small_blocks[i]) {
            BuddyAllocator_free_metabuddy(allocator, small_blocks[i]);
        }
    }
    
    // Try to allocate a bigger block
    void* big_block = BuddyAllocator_malloc_metabuddy(allocator, 512);
    check(big_block != NULL, "allocated larger block after freeing smaller ones");
    ...
    if (big_block) {
        BuddyAllocator_free_metabuddy(allocator, big_block);
    }
...
void test_edge_cases_metabuddy() {
    ...
    // Try allocating 0 bytes - what should happen?
    void* zero_ptr = BuddyAllocator_malloc_metabuddy(allocator, 0);
    check(zero_ptr == NULL, "allocating 0 bytes returns NULL");

    // Try a really large allocation
    void* huge_ptr = BuddyAllocator_malloc_metabuddy(allocator, 1024 * 1024 * 2); // 2MB
    check(huge_ptr == NULL, "huge allocation fails and returns NULL");

    // Try freeing NULL pointer (shouldn't crash)
    BuddyAllocator_free_metabuddy(allocator, NULL);
    check(1, "freeing NULL doesn't crash");
